					...
					<div class="d-flex justify-content-between align-items-center">
						...
						<div>
							<button type="submit" class="btn btn-primary btn-sm"
								form="checkForm-${q.questId}">체크 저장</button>

							<button type="submit"
								class="btn btn-success btn-sm ms-2 coin-btn"
								form="coinForm-${q.questId}" data-questid="${q.questId}"
								${percent ne 100 ? 'disabled' : ''}>코인 지급</button>
						</div>
					</div>

					<div class="map-placeholder mt-2">위치: ${q.address}
						(${q.latitude}, ${q.longitude})</div>
				</div>
			</div>
		</c:forEach>
	</div>
